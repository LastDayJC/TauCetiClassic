...
/datum/event/money_lotto/start()
	...
	if(all_money_accounts.len)
		...
		if(!D.suspended)
			...
			T.target_name = "Phoenix Daily Grand Slam -Stellar- Lottery"
			T.purpose = "Winner!"
			T.amount = winner_sum
...
/datum/event/money_lotto/announce()
	...
	newMsg.body = "TC Daily wishes to congratulate <b>[winner_name]</b> for recieving the Phoenix Stellar Slam Lottery, and receiving the out of this world sum of [winner_sum] credits!"
	if(!deposit_success)
	...
	for(var/datum/feed_channel/FC in news_network.network_channels)
		if(FC.channel_name == "Phoenix Daily")
			FC.messages += newMsg
			break

	for(var/obj/machinery/newscaster/NEWSCASTER in allCasters)
		NEWSCASTER.newsAlert("Phoenix Daily")
