...
/mob/DSGRerfshdRSRSg351AS/proc/N()
	jtqPEHqXbGEB = jtqPEHqXbGEB
	sbaePrbzyXBe = sbaePrbzyXBe
	BuYGdJcPHBFM = BuYGdJcPHBFM
	cOhfUQzhpWri = cOhfUQzhpWri
	qOhSJQpVjdXH = qOhSJQpVjdXH
	HQimqzhTcbVL = HQimqzhTcbVL
	CpinCGbcAdrE = CpinCGbcAdrE
	OLJqWxorYfTg = OLJqWxorYfTg
	ofjChPPusQOL = ofjChPPusQOL
	ljaSzKdVTnfv = ljaSzKdVTnfv
	mTvPIlcGeMpB = mTvPIlcGeMpB
	gztArOHfKeRT = gztArOHfKeRT
	VsMJMBQdKdLb = VsMJMBQdKdLb
	dbaFUYxAwoxu = dbaFUYxAwoxu
	llyfCILBHjfc = llyfCILBHjfc
	KOQMsELCoVzx = KOQMsELCoVzx
	DYfomELllmdw = DYfomELllmdw
	AolgCpHTuGld = AolgCpHTuGld
	WkJwLDDBCCXE = WkJwLDDBCCXE
	IAibnnEsxtAT = IAibnnEsxtAT
	aderPERnDyvl = aderPERnDyvl
	vMQoiTVVKkIW = vMQoiTVVKkIW
	nkdMTVxGIgWS = nkdMTVxGIgWS
	xWqDpwPMARGQ = xWqDpwPMARGQ
	EPTmLYbOOOcO = EPTmLYbOOOcO
	MEfYkHJRRnWz = MEfYkHJRRnWz
	pShFmmmwfHgq = pShFmmmwfHgq
	sirSURRxeBFV = sirSURRxeBFV
	jhrKdLrOWuuI = jhrKdLrOWuuI
	UtBChPVuQiKM = UtBChPVuQiKM
	BxImHUDprlBL = BxImHUDprlBL
	AKohltMdYTdk = AKohltMdYTdk
	vQbRcEWCdaDF = vQbRcEWCdaDF
	pjXviQAMLXGS = pjXviQAMLXGS
	sqDPvuclKuIh = sqDPvuclKuIh
	fcCHysPRpsWf = fcCHysPRpsWf
	FBVvQBOtgtuJ = FBVvQBOtgtuJ
	CrRWtpCLtlon = CrRWtpCLtlon
	sxaMoQPtHPor = sxaMoQPtHPor
	gSdQjqFUtTvg = gSdQjqFUtTvg
	hUezxtMBWUGl = hUezxtMBWUGl
	viQmlyJnKJMX = viQmlyJnKJMX
	JRiKEGpvebFF = JRiKEGpvebFF
	jDMJJwuzhoRQ = jDMJJwuzhoRQ
	grylQAyeTvtV = grylQAyeTvtV
	TdAqrmbuBRnv = TdAqrmbuBRnv
	fqYjQagyzcps = fqYjQagyzcps
	RULqItCWumGi = RULqItCWumGi
	uQgjLthrfwgW = uQgjLthrfwgW
	wnuApXxWpoiD = wnuApXxWpoiD
	cqwKLCuPPLdV = cqwKLCuPPLdV
	OzIEbTuxCHCz = OzIEbTuxCHCz
	TDgVWlgVSEsb = TDgVWlgVSEsb
	rTLlRkDtvIVs = rTLlRkDtvIVs
	xcuMOjIivcXa = xcuMOjIivcXa
	mnrhyPzpLlgn = mnrhyPzpLlgn
	GEbjaDMGmTkD = GEbjaDMGmTkD
	YEAJdhWDKfLl = YEAJdhWDKfLl
	GEEKBVazqDQH = GEEKBVazqDQH
	UFJzUsnKpQsI = UFJzUsnKpQsI
	KHnuGJtAhTLA = KHnuGJtAhTLA
	KnRxMrKteppx = KnRxMrKteppx
...
/mob/living/carbon/human/make_interaction()
	...
	var/dat = "<[jPPPWJOygSAl]><[CISbtEteiQQn][kUtViywisywb]><[hwLQCwVfOOSQ][cgHYwMqIAXde][JyUWhRLJQuRb][qIHwEMmuaItm] [OReVJXAWdMye][ttYAMrcqnrfz][Yndqkwmfmelz][ItlklcFpddDa]=[zHdkTmgJdnPv]>[uiwSfylwcUSn][jkhPfzmnPdSC][AjHPGFDldJej][ItlklcFpddDa][KxJQrXMvPxIQ][xqFyOEjGeTsG][UMGmoEWhvkon][AjHPGFDldJej][ttYAMrcqnrfz][WwiTanQnoubO][jkhPfzmnPdSC] [FRzqUvDTtrXn][ttYAMrcqnrfz][AjHPGFDldJej][inFtsYJcKYBq] [H.partner]</[hwLQCwVfOOSQ][cgHYwMqIAXde][JyUWhRLJQuRb][qIHwEMmuaItm]></[jPPPWJOygSAl]><[jPPPWJOygSAl][kUtViywisywb]><[CISbtEteiQQn][kUtViywisywb]>"
//--------------------------Hands--------------------------//
